    ...
    <div class="store-container">
      ...
      <main class="product-gallery">
        <div class="gallery-header">
          <h2>สินค้าหมวด: <span class="text-neon">{{ selectedTeam }}</span></h2>
        </div>
...
// ฟิลเตอร์สินค้าตามทีมที่เลือก
const filteredProducts = computed(() => {
  if (selectedTeam.value === 'ทั้งหมด (All Teams)') {
    return products.value
  }
  return products.value.filter(p => p.team === selectedTeam.value)
})

// ฟังก์ชันกำหนดสีพื้นหลังของรูปสินค้าตามทีม
...
.product-name {
  ...
  color: #f8fafc;
  line-height: 1.4;
  height: 48px; /* ล็อกความสูงให้การ์ดเท่ากัน */
  overflow: hidden;
}
